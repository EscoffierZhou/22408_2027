...
目的[1]:建立考研线性代数宏观知识全景，深刻理解“向量空间为基石、矩阵为算子、线性方程组为核心”的现代代数结构

目的[2]:掌握向量的定义、维数、实际信息载体功能以及线性运算（向量加法、数乘向量）的代数与几何意义

目的[3]:融会贯通向量点积（内积）运算规则，掌握一行乘一列、一行乘多列、多行乘一列及多行乘多列的矩阵乘法演进阶梯

目的[4]:深刻理解矩阵的语义锁定特性与“算子映射观”，掌握二维平面五大基础几何变换（对称、伸缩、剪切、旋转）的矩阵表达与作用机理

目的[5]:掌握线性方程组的矩阵表达 $Ax=b$，对比高斯消元法与大学逆矩阵算子解法的深层哲学，树立线性系统逆映射解法思维
...
>行列式、矩阵、矩阵运算与初等变换。
>
>提供全书的基础运算法则与运算载体，是后续一切代数分析的基本功。

[2]主题版块（代数结构核心骨架）：
>向量组、线性相关性、线性方程组的解空间。
>
>深入研究向量空间的基、维数、线性表出与齐次/非齐次方程组解的结构定理。

[3]应用版块（几何主轴化与坐标变换）：
>特征值与特征向量、相似对角化、二次型。
>
>通过正交变换化二次型为标准形，解决代数与几何空间中的主轴化与极值判定。
...
>
>通俗而言，我们在向量空间这一统一步骤中，深入探究<font color=red>向量与向量之间的关系</font>、<font color=deeppink>空间到空间的映射法则</font>，以及<font color=red>方程系统的代数解集结构</font>。
...
>(3) 喜欢：记为 $1$。
>
>此时即可构建表征某人兴趣偏好的 2 维特征向量：$[\text{运动兴趣程度}, \text{音乐兴趣程度}]$。
>
...
>
>在二维与三维欧几里得空间中，向量加法服从**平行四边形法则**或**三角形连加法则**。
...
[1]高数与线代的映射对齐结构：
>在高等数学中，核心研究工具是单变量或多变量函数映射：
>
>$$y = f(x)$$
>
>而在高等代数中，核心工具全面升级为矩阵与向量的线性作用：
>
>$$A\alpha = \beta$$
>
>两者具有高度对齐的映射结构：
...
[2]方法优劣分析：
>在未知数较少时非常直观，容易手工完成；但在大型复杂系统或需要反复对不同常数向量 $b$ 求解时，消元过程高度依赖具体数值，缺乏全局结构解。
