...
\begin{itemize}
  \item 目的[1]:掌握原函数与不定积分的基本概念、原函数存在定理及导函数的形态限制(达布定理)
\end{itemize}
\end{targetbox}

\begin{targetbox}[本章复习目标与核心知识图谱]
\begin{itemize}
  \item 目的[2]:掌握定积分黎曼和定义、几何代数意义及可积性的必要条件与四大充分条件
\end{itemize}
\end{targetbox}

\begin{targetbox}[本章复习目标与核心知识图谱]
\begin{itemize}
  \item 目的[3]:熟练运用定积分保号性、估值定理、积分第一中值定理分析积分等式与不等式
\end{itemize}
\end{targetbox}

\begin{targetbox}[本章复习目标与核心知识图谱]
\begin{itemize}
  \item 目的[4]:深刻理解变上限积分函数的连续性、可导性与间断点传导规律(积分升阶定理)
\end{itemize}
\end{targetbox}

\begin{targetbox}[本章复习目标与核心知识图谱]
\begin{itemize}
  \item 目的[5]:掌握反常积分(无穷区间型与瑕积分)敛散性判别法、两大标准p-积分基准与阶数压制模型
\end{itemize}
\end{targetbox}


\section{1.不定积分与原函数存在性}

\begin{thmbox}[概念[1]: 原函数与不定积分的基本概念]

\textbf{[1]} 原函数定义: 设函数 $f(x)$ 定义在区间 $I$ 上，若存在可导函数 $F(x)$，对于该区间上任意一点都有 $F'(x) = f(x)$，则称 $F(x)$ 是 $f(x)$ 在区间 $I$ 上的一个原函数。

\textbf{[2]} 不定积分定义: 称 $\int f(x)\,\mathrm{d}x = F(x) + C$ 为 $f(x)$ 在区间 $I$ 上的不定积分(表示全体原函数的集合)。

注: 谈到原函数与不定积分，必须指明定义区间。

\end{thmbox}
\begin{thmbox}[定理[1]: 原函数(不定积分)存在定理与导函数的形态限制]

\textbf{[1]} 存在充分条件(必有): 若 $f(x)$ 在区间 $I$ 上连续，则 $f(x)$ 必有原函数。

构造性证明: 设 $F(x) = \int_a^x f(t)\,\mathrm{d}t$(变上限积分)，由积分中值定理可证 $F'(x) = f(x)$。

\textbf{[2]} 不存在充分条件(必无): 含有第一类间断点(可去、跳跃)或无穷间断点的函数，在包含该间断点的区间内必无原函数。

\textbf{[1]} 达布定理(导数介值性): 若 $F(x)$ 在 $[a, b]$ 可导，则导函数 $F'(x)$ 在区间内介于任意两导数值之间的值都能取到，因此导函数不能出现跳跃。

\textbf{[2]} 洛必达求导法则的反证: 若假设可去、跳跃或无穷间断点处存在原函数，根据导数定义 $\lim\limits_{x \to x_0} \frac{F(x)-F(x_0)}{x-x_0} \stackrel{\text{洛}}{=} \lim\limits_{x \to x_0} F'(x)$，必导出矛盾。

\textbf{[3]} 不定情况(可能有，也可能没有): 含有振荡间断点的函数。

\textbf{[1]} 有原函数的例子: $f(x) = \begin{cases} 2x\sin\frac{1}{x} - \cos\frac{1}{x}, & x \neq 0 \\ 0, & x = 0 \end{cases}$，原函数为 $F(x) = \begin{cases} x^2\sin\frac{1}{x}, & x \neq 0 \\ 0, & x = 0 \end{cases}$。

\textbf{[2]} 无原函数的例子: $f(x) = \begin{cases} \frac{1}{x}\sin\frac{1}{x}, & x \neq 0 \\ 0, & x = 0 \end{cases}$。

\end{thmbox}
\begin{thmbox}[原理[1]: \textbf{【核心直观与小故事总结】}导数为什么不会突变？振荡间断点真的“断”了吗？]

关于连续、导数与振荡间断点的微观几何小故事：

\textbf{[1]} 导函数的“点点相依”: 普通函数的值可以脱节独立(如跳跃点、孤立点)，但导数是差商的极限，它的存在天然要求两点之间“依附并充分靠近”。因此，如果导函数在某点的极限存在，这个极限值必须严格等于该点的导数值，导函数绝不可能产生第一类突变。

\textbf{[2]} 切线不会瞬时折断: 几何上，一条处处有切线的平滑曲线，切线斜率绝对无法在某一点发生瞬时跳跃。

\textbf{[3]} 振荡间断点并没有“断开”: 以典型反例 $f(x) = \sin\frac{1}{x}$ 为例，它被称为“间断”，仅仅是因为极限 $\lim\limits_{x \to 0} \sin\frac{1}{x}$ 不存在，不满足微积分中严格的“极限值 = 函数值”代数定义。若在显微镜下把 $x$ 轴无限放大无穷倍，其曲线仍然是一条连续蜿蜒、绵延不绝的完整曲线，点与点之间从未像跳跃间断那样撕开。正因如此，可导函数的导函数才被允许拥有这种虽然剧烈抖动、但实则紧密相连的“振荡间断”。

\end{thmbox}

\section{2.定积分的概念、几何意义与可积性}


\begin{equation*}
\int_a^b f(x)\,\mathrm{d}x = \lim_{\lambda \to 0} \sum_{k=1}^n f(\xi_k)\Delta x_k
\end{equation*}
\begin{thmbox}[概念[1]: 黎曼定积分的定义(四步法)]

设函数 $f(x)$ 在闭区间 $[a, b]$ 上有界。

\textbf{[1]} 分割: 在 $(a, b)$ 内任意插入 $n-1$ 个分点 $x_i$，将 $[a, b]$ 分成 $n$ 个小区间 $[x_{k-1}, x_k]$，记小区间的长度 $\Delta x_k = x_k - x_{k-1}$，并记模 $\lambda = \max\limits_{1 \leqslant k \leqslant n} \{\Delta x_k\}$。

\textbf{[2]} 近似: 在每个小区间 $[x_{k-1}, x_k]$ 上任取一点 $\xi_k$，用矩形面积近似曲边梯形小条: $f(\xi_k)\Delta x_k$。

\textbf{[3]} 求和: 将所有小矩形面积累加，得到黎曼和 $\sum\limits_{k=1}^n f(\xi_k)\Delta x_k$。

\textbf{[4]} 取极限: 当 $\lambda \to 0$ 时，若极限 $\lim\limits_{\lambda \to 0} \sum\limits_{k=1}^n f(\xi_k)\Delta x_k$ 存在，且该极限值与分点 $x_i$ 的划分法及点 $\xi_k$ 的取法均无关，则称 $f(x)$ 在 $[a, b]$ 上黎曼可积，此极限值称为定积分：


\end{thmbox}
\begin{thmbox}[原理[1]: 历史与符号背景小记]

\textbf{[1]} 积分符号 $\int$ 由德国数学家莱布尼茨(G. W. Leibniz)创立，源于拉丁文“总和(summa)”的首字母 $S$ 的拉长变形。

\textbf{[2]} 定积分的现代严格定义是由德国数学家黎曼(B. Riemann)给出的，故通常称为黎曼积分。

\end{thmbox}
\begin{thmbox}[原理[2]: 定积分的几何意义(代数和)]

\textbf{[1]} 若 $f(x) \geqslant 0$，$\int_a^b f(x)\,\mathrm{d}x$ 表示曲边梯形的面积 $S$。

\textbf{[2]} 若 $f(x) \leqslant 0$，$\int_a^b f(x)\,\mathrm{d}x$ 表示曲边梯形面积的负值 $-S$。

\textbf{[3]} 若 $f(x)$ 符号变动，$\int_a^b f(x)\,\mathrm{d}x = S_{\text{上}} - S_{\text{下}}$($x$ 轴上方图形面积减去下方图形面积)。
...
\end{equation*}
\begin{thmbox}[方法[1]: 经典特取法与“定积分定义求极限”的标准型]
...
\textbf{[1]} 提公因子 $\frac{1}{n}$(视作 $\mathrm{d}x$)；

\textbf{[2]} 凑自变量形如 $\frac{i}{n}$(视作 $x$)；

\textbf{[3]} 确定求和指标范围，转化为 $\int_0^1 f(x)\,\mathrm{d}x$。
...
\end{equation*}
\begin{thmbox}[原理[3]: 积分与哑变量无关]


定积分的值是一个确定的实数值，仅取决于被积表达式与积分区间，与积分变量所用的字母符号无关。

\end{thmbox}
\begin{thmbox}[定理[1]: 可积性(定积分存在定理)]

常义定积分的立足基础是区间有限且函数有界。

\textbf{[1]} 必要条件: 若 $f(x)$ 在 $[a, b]$ 上可积，则 $f(x)$ 在 $[a, b]$ 上必有界。

反之不成立: 有界函数不一定可积(例如狄利克雷函数在 $[0,1]$ 上有界但处处不可积)。

\textbf{[2]} 充分条件(满足其一即必可积):

\textbf{[1]} $f(x)$ 在 $[a, b]$ 上连续；

\textbf{[2]} $f(x)$ 在 $[a, b]$ 上单调(单调函数必有界)；

\textbf{[3]} $f(x)$ 在 $[a, b]$ 上有界，且只有有限个间断点；

\textbf{[4]} $f(x)$ 在 $[a, b]$ 上只有有限个第一类间断点(第一类间断点处必局部有界)。
...
\textbf{[2]} 区间长度: $\int_a^b 1\,\mathrm{d}x = b - a$

\textbf{[3]} 线性性质: $\int_a^b [k_1 f(x) \pm k_2 g(x)]\,\mathrm{d}x = k_1\int_a^b f(x)\,\mathrm{d}x \pm k_2\int_a^b g(x)\,\mathrm{d}x$

\textbf{[4]} 区间可加性: 无论 $a, b, c$ 相对大小如何，总有 $\int_a^b f(x)\,\mathrm{d}x = \int_a^c f(x)\,\mathrm{d}x + \int_c^b f(x)\,\mathrm{d}x$

\textbf{[5]} 保号性与单调性:

\textbf{[1]} 若在 $[a, b]$ 上 $f(x) \leqslant g(x)$(且 $a < b$)，则 $\int_a^b f(x)\,\mathrm{d}x \leqslant \int_a^b g(x)\,\mathrm{d}x$。

\textbf{[2]} 绝对值不等式: $\left\vert{}\int_a^b f(x)\,\mathrm{d}x\right\vert{} \leqslant \int_a^b \vert{}f(x)\vert{}\,\mathrm{d}x$。

\textbf{[3]} 严格不等式推论: 设 $f(x)$ 在 $[a, b]$ 上连续、非负($f(x) \geqslant 0$)且 $f(x) \not\equiv 0$，则 $\int_a^b f(x)\,\mathrm{d}x > 0$。

\textbf{[6]} 估值定理: 设 $M, m$ 分别为 $f(x)$ 在 $[a, b]$ 上的最大值和最小值，$L = b - a$，则：


\textbf{[7]} 积分中值定理: 若 $f(x)$ 在 $[a, b]$ 上连续，则至少存在一点 $\xi \in [a, b]$，使得：
...
\end{thmbox}
\begin{thmbox}[定理[1]: 关键性质与平滑性规律(积分升阶定理)]

变限积分运算会使原函数的性质“提升一级”：

\textbf{[1]} $f(x)$ 可积 $\implies F(x)$ 必处处连续:

若 $f(x)$ 在区间 $I$ 上可积(例如有界且仅含有限个间断点)，则 $F(x) = \int_a^x f(t)\,\mathrm{d}t$ 在区间 $I$ 上处处连续。

\textbf{[2]} $f(x)$ 连续 $\implies F(x)$ 处处可导:
...
\textbf{[3]} 被积函数间断点对变限积分可导性的映射规律:

\textbf{[1]} 跳跃间断点 $\to$ 尖点(不可导点):
...
\begin{thmbox}[概念[1]: 无穷区间反常积分]

\textbf{[1]} 单侧无穷上限: $\int_a^{+\infty} f(x)\,\mathrm{d}x = \lim\limits_{x \to +\infty} F(x) - F(a)$(极限存在则收敛，否则发散)

\textbf{[2]} 单侧无穷下限: $\int_{-\infty}^b f(x)\,\mathrm{d}x = F(b) - \lim\limits_{x \to -\infty} F(x)$

\textbf{[3]} 双侧无穷区间: $\int_{-\infty}^{+\infty} f(x)\,\mathrm{d}x = \int_{-\infty}^{c} f(x)\,\mathrm{d}x + \int_c^{+\infty} f(x)\,\mathrm{d}x$

两端必须同时收敛才收敛，只要有一侧发散即严格发散。

\end{thmbox}
\begin{thmbox}[概念[2]: 无界函数的反常积分(瑕积分)]

若被积函数在积分点附近趋向无穷，该点称为瑕点：

\textbf{[1]} $x = a$ 为唯一瑕点: $\int_a^b f(x)\,\mathrm{d}x = F(b) - \lim\limits_{x \to a^+} F(x)$

\textbf{[2]} $x = b$ 为唯一瑕点: $\int_a^b f(x)\,\mathrm{d}x = \lim\limits_{x \to b^-} F(x) - F(a)$

\textbf{[3]} $x = c \in (a, b)$ 为区间内部瑕点: $\int_a^b f(x)\,\mathrm{d}x = \int_a^c f(x)\,\mathrm{d}x + \int_c^b f(x)\,\mathrm{d}x$(两侧必须均收敛)

\end{thmbox}
\begin{thmbox}[定理[1]: 敛散性比较判别法与极限形式]

设 $f(x), g(x)$ 非负：

\textbf{[1]} 比较审敛准则: 大收则小收，小发则大发。

\textbf{[2]} 极限形式(设 $\lim \frac{f(x)}{g(x)} = \lambda$):

\textbf{[1]} $0 < \lambda < +\infty$: $\int f$ 与 $\int g$ 同敛散；

\textbf{[2]} $\lambda = 0$: 若 $\int g$ 收敛，则 $\int f$ 必收敛；

\textbf{[3]} $\lambda = +\infty$: 若 $\int g$ 发散，则 $\int f$ 必发散。

\end{thmbox}
\begin{thmbox}[定理[2]: 考研必背的两大标准 $p$-积分]

| \textbf{类型} | \textbf{积分形式} | \textbf{收敛条件} | \textbf{发散条件} | \textbf{记忆直观内核} |
| :--- | :--- | :--- | :--- | :--- |
| \textbf{无穷区间型} | $\int_a^{+\infty} \frac{1}{x^p}\,\mathrm{d}x \quad (a > 0)$ | \textit{}$p > 1$\textbf{ | }$p \leqslant 1$\textit{} | 趋向无穷大时，分母次数越大，衰减到 $0$ 的速度越快，面积越容易收敛。 |
| \textbf{瑕积分型} | $\int_0^a \frac{1}{x^p}\,\mathrm{d}x \quad (a > 0)$ | \textit{}$0 < p < 1$\textbf{ | }$p \geqslant 1$\textit{} | 趋向零点时，分母次数越小，冲向无穷大的速度越慢，面积越容易被圈住收敛。 |

对称区间奇偶性结论(若反常积分 $\int_0^{+\infty} f(x)\,\mathrm{d}x$ 收敛)：

\textbf{[1]} $f(x)$ 为偶函数 $\implies \int_{-\infty}^{+\infty} f(x)\,\mathrm{d}x = 2\int_0^{+\infty} f(x)\,\mathrm{d}x$

\textbf{[2]} $f(x)$ 为奇函数 $\implies \int_{-\infty}^{+\infty} f(x)\,\mathrm{d}x = 0$

\end{thmbox}

\section{5.核心解题方法体系总结}
...
\end{align*}
\begin{thmbox}[方法[1]: 反常积分审敛的“阶数压制”与四大基准型]
...
\textbf{[3]} 补充(复合对数型，利用直接积分):


\end{thmbox}
\begin{thmbox}[方法[2]: “定积分定义求和式极限”标准操作SOP]

遇到 $n$ 项和极限 $\lim\limits_{n \to \infty} \sum$，判定依据：

\textbf{[1]} 定积分定义法标志: 分子分母中变量 $i$ 与 $n$ 的最高次数相同，能凑出公因子 $\frac{1}{n}$ 与变量 $\frac{i}{n}$。

\textbf{[1]} 提公因子: 提取 $\frac{1}{n} \to \mathrm{d}x$；

\textbf{[2]} 换元映射: 将余下表达式中的 $\frac{i}{n}$ 全换为自变量 $x$；

\textbf{[3]} 确定积分限: 求和范围 $i=1 \sim n$ 对应积分区间 $[0, 1]$。

\textbf{[2]} 夹逼准则标志: 首尾项差异小，或者无法凑出统一的 $\frac{i}{n}$ 时，先放缩分子或分母后应用夹逼准则。
...
\textbf{\color{DeepPurple}【思路点拨】} 两端关于自变量 $x$ 求导消去不定积分号，解出被积复合表达式，再通过简单换元还原函数 $f(x)$
...
\textbf{[4]} 故正确选项为 \textbf{(B)}。
\end{solbox}


\subsection{例题 2: 分段函数的原函数求解与分段常数匹配}
...
\textbf{\color{DeepPurple}【思路点拨】} 对两侧区间分别求不定积分引入独立常数，根据原函数在分段点处必须严格连续确定分段常数间的约束关系
...
\textbf{\color{DeepPurple}【思路点拨】} 紧扣原函数存在定理(达布定理与第一类间断点必无原函数)与定积分存在定理(有界性必要条件与有限间断点充分条件)逐一研判
...
\textbf{[5]} 综上，正确的命题为 ①②③，共 3 个。故正确选项为 \textbf{(C)}。
\end{solbox}


\subsection{例题 4: 严格递增反函数积分与矩形面积大小比较(杨不等式几何直观)}
...
\textbf{\color{DeepPurple}【思路点拨】} 将两个定积分分别对应为以 $x$ 轴与以 $y$ 轴为底的曲边梯形面积，根据反函数单调性及点与曲线位置关系分析几何重叠
...
\textbf{[4]} 故正确选项为 \textbf{(A)}。
\end{solbox}


\subsection{例题 5: 振荡衰减函数拱形面积级数求和}

曲线 $y = e^{-x}\sin x$ 在 $[0, +\infty)$ 上与 $x$ 轴围成的所有拱形区域的总面积记为 $S$，求其表达式与具体数值。

\textbf{\color{DeepPurple}【思路点拨】} 定积分几何面积需加绝对值，将全区间按正弦零点拆分为无穷多个正负交替的小拱形，计算单拱面积后转化为无穷等比级数求和
...
\textbf{[5]} 结论：总面积表达式为 $\int_0^{+\infty} e^{-x}\vert{}\sin x\vert{}\,\mathrm{d}x = \frac{e^\pi + 1}{2(e^\pi - 1)}$。
\end{solbox}


\subsection{例题 6: 定积分定义求解连和式极限的标准型转化}
...
\textbf{\color{DeepPurple}【思路点拨】} 用 $\Sigma$ 符号写出紧凑通项，提公因子 $\frac{1}{n}$ 对应微分 $\mathrm{d}x$，凑自变量 $\frac{i}{n}$ 对应 $x$，由定积分定义完成转化


\begin{solbox}[分步推导与答题规范]
\textbf{[1]} 用求和符号表示并提公因子：
...
\subsection{例题 7: 非负连续非恒零函数积分严格保号性证明}

设 $f(x)$ 在 $[a, b]$ 上非负连续，且 $f(x) \not\equiv 0$，证明：$\int_a^b f(x)\,\mathrm{d}x > 0$。

\textbf{\color{DeepPurple}【思路点拨】} 因非恒零必存在一处严格大于零的点，由连续函数局部保号性锁定一个闭正邻域，将全区间积分拆分放缩，说明积分值严格大于零


\begin{solbox}[分步推导与答题规范]
\textbf{[1]} 锁定正值点并应用局部保号性：
...
\textbf{[3]} 结论：故严格证得 $\int_a^b f(x)\,\mathrm{d}x > 0$。
\end{solbox}


\subsection{例题 8: 积分第一中值定理与介值定理严格证明}

设 $f(x)$ 在 $[a, b]$ 上连续，证明：存在 $\xi \in [a, b]$，使得 $\int_a^b f(x)\,\mathrm{d}x = f(\xi)(b-a)$。

\textbf{\color{DeepPurple}【思路点拨】} 利用闭区间连续函数必能取得最值进行估值，由介值定理直接说明均值必能在区间上被取得
...
\subsection{例题 9: 定积分大小比较与被积函数不等式链}
...
\textbf{\color{DeepPurple}【思路点拨】} 在区间 $(0, \frac{\pi}{4})$ 内利用经典几何三角不等式链比较被积函数大小，结合定积分保序性直接排序
...
\subsection{例题 10: 三角复合函数单调性与区间对称换元比较}
...
\textbf{\color{DeepPurple}【思路点拨】} 对 $M$ 利用 $\sin x < x$ 及正弦单调性放缩；对 $N$ 进行区间再现换元 $x = \frac{\pi}{2} - t$，利用余弦单调性放缩
...
\subsection{例题 11: 间断点在变上限积分中的连续性与可导性传导}

设函数 $y = f(x)$ 在 $[-1, 3]$ 上的图像已知，其中 $x = 0$ 为可去间断点，$x = 2$ 为跳跃间断点。分析变上限积分 $F(x) = \int_0^x f(t)\,\mathrm{d}t$ 的图形特征。

\textbf{\color{DeepPurple}【思路点拨】} 根据变上限积分平滑性定理，有界可积函数的变上限积分处处连续，可去间断点处光滑可导，跳跃间断点处不可导（呈现尖点）
...
\textbf{[4]} 结论：$F(x)$ 曲线在全区间处处连续，在 $x = 0$ 处光滑可导，在 $x = 2$ 处不可导且形成尖点。
\end{solbox}


\subsection{例题 12: 分段函数变上限积分的可导性与导数计算}
...
\textbf{\color{DeepPurple}【思路点拨】} 检查分界点处被积函数的间断点类型，结合变上限积分对间断点的升阶作用进行判定


\begin{solbox}[分步推导与答题规范]
\textbf{[1]} 判定被积函数在分界点的性态：
...
\textbf{[3]} 结论：$F(x)$ 在 $x = \pi$ 处连续但不可导。

\textbf{[4]} 故正确选项为 \textbf{(D)}。
\end{solbox}


\subsection{例题 13: 可去间断点函数变上限积分的处处可导性}

设 $f(x) = \begin{cases} e^{x^2} + x^2, & x \neq 0 \\ a, & x = 0 \end{cases}$，$a$ 为实常数，$F(x) = \int_0^x f(t)\,\mathrm{d}t$。讨论 $F(x)$ 在 $x = 0$ 处的可导性。

\textbf{\color{DeepPurple}【思路点拨】} 利用导数定义转化为极限，被积函数在可去间断点处的极限存在即可保证变限积分可导，其导数值与孤立点定义值 $a$ 无关
...
\subsection{例题 14: 变上限积分函数有界性证明(结合积分均值与指数衰减)}

设 $a > 0$，$f(x)$ 在 $[0, +\infty)$ 上有界，$C$ 为任意常数。证明：$y = e^{-ax}\left[ \int_0^x f(t)e^{at}\,\mathrm{d}t + C \right]$ 在 $[0, +\infty)$ 上有界。

\textbf{\color{DeepPurple}【思路点拨】} 设出函数界的正数 $M$，将衰减因子 $e^{-ax}$ 乘入积分号内部，利用三角不等式与直接积分进行上界放大
...
\subsection{例题 15: 双奇点反常积分的拆分与参数收敛充要条件}
...
\textbf{\color{DeepPurple}【思路点拨】} 识别积分的两个奇点 $x = 0$(瑕点)与 $x = +\infty$(无穷)，拆分为两段分别由低次项和高次项主导，应用两大标准 $p$-积分判定
...
\subsection{例题 16: 等价无穷小代换与反常积分参数审敛}

反常积分 $\int_1^{+\infty} \left( e^{\frac{\cos\frac{1}{x}}{x}} - e^{\frac{1}{x}} \right) x^k\,\mathrm{d}x$ 收敛，则参数 $k$ 的取值范围是 $\underline{\hspace{2.5cm}}$。

\textbf{\color{DeepPurple}【思路点拨】} 当 $x \to +\infty$ 时 $\frac{1}{x} \to 0$，利用等价无穷小与泰勒展开提取主部，比照无穷区间 $p$-积分基准确定参数范围
...
\textbf{[4]} 故参数 $k$ 的取值范围为 $k < 2$。
\end{solbox}


\subsection{例题 17: 反常积分敛散性的综合比较与判定}
...
\textbf{\color{DeepPurple}【思路点拨】} 逐项检查各积分奇点处的阶数与渐近行为，运用泰勒展开及基准 $p$-积分判断敛散性
...
\subsection{例题 18: 瑕积分对数基准型的参数分类讨论}
...
\textbf{\color{DeepPurple}【思路点拨】} 应用分部积分法求出带有参数的原函数表达式，利用洛必达法则求极限，分析其收敛的充要条件
...
\subsection{例题 19: 无穷区间对数反常积分的阶数压制与参数判定}
...
\textbf{\color{DeepPurple}【思路点拨】} 运用对数是“最弱发散函数”的阶数压制法则，构造介于 $1$ 与 $\alpha$ 之间的微小量进行极限形式比较审敛


\begin{solbox}[分步推导与答题规范]
\textbf{[1]} 当 $\alpha > 1$ 时的收敛性证明：
...
\textbf{[2]} 当 $0 < \alpha \leqslant 1$ 时的发散性证明：
