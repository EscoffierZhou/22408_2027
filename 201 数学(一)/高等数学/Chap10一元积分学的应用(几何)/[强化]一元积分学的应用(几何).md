...
目的[1]:攻克平面任意倾斜直线轴 $Ax+By+C=0$ 旋转体体积与侧面积的通用微分积分模型（通用公式法与投影变形补偿法则）

目的[2]:深刻掌握古尔丁第一定理（Pappus-Guldin 弧线转动侧面积定理）与古尔丁第二定理（平面转动体积定理），实现形心与旋转几何量秒算互逆

目的[3]:攻克极坐标曲线绕极轴及过原点任意射线的旋转体体积与侧面积公式推导与极简积分

目的[4]:掌握已知复杂倾斜平行截面面积（椭圆截面、切角棱柱）的柱体与锥体体积高阶切片积分

目的[5]:深刻掌握无穷区间反常几何量（反常面积、反常旋转体体积、反常弧长）与级数敛散性交织的综合大题

目的[6]:掌握形心坐标、质心转动惯量与一阶/二阶矩在变力做功和液体压力中的物理-几何交叉渗透

## 1.斜轴旋转体体积的通用积分公式与几何补偿本质

###### **模型[1]:绕任意一般式直线 $Ax+By+C=0$ 旋转体积通用公式(10-1)的严格推导**
...
[3]退化特例的精确自洽性验证：
>(1) **绕 $x$ 轴旋转**：直线方程为 $y = 0$，即 $A = 0, B = 1, C = 0$。代入公式：
>
>$$V = \frac{\pi}{(0+1)^{3/2}} \int_a^b (y(x))^2 |1 - 0|\,\mathrm{d}x = \pi \int_a^b y^2(x)\,\mathrm{d}x$$
>
>完全退化为标准圆盘法公式！
>
>(2) **绕 $y$ 轴旋转**：直线方程为 $x = 0$，即 $A = 1, B = 0, C = 0$。代入公式：
>
...
###### **模型[2]:倾斜圆盘微元的几何补偿与斜交投影误差对消（板书 $\alpha = \beta$ 深度剖析）**
...
>
>秒解半圆盘形心纵坐标：$$\overline{y} = \frac{4R}{3\pi}$$
>

## 3.极坐标系下的旋转体体积与侧面积

###### **模型[5]:极坐标图形绕极轴与垂直射线旋转的体积微元公式**
...
[2]绕垂直射线（$\theta = \frac{\pi}{2}$，即 $y$ 轴）旋转：
...
###### **模型[7]:双圆柱直交相交体（牟合方盖 Steinmetz Solid）的体积与侧面积**
...
>
>因此，在垂直于 $x$ 轴的截面处，形状为一个边长为 $2\sqrt{R^2-x^2}$ 的**正方形**！
>
>截面面积为：
>
...
###### **模型[8]:倾斜截角圆柱体与水槽切面问题（Cavalieri 楔体）**
...
## 5.反常积分与无穷几何量级数敛散性联动模型

###### **模型[9]:托里拆利小号（Gabriel's Horn）悖论与反常几何量的 $p$ 级数本质**
