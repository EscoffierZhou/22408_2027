...
目的[1]:熟练掌握基本积分公式系统及13组高频凑微分模型，做到见形拆招、正反双向秒切

目的[2]:深刻领会第二类换元法(三角代换、根式代换、倒代换)与分部积分法("反对幂指三"、表格十字相乘法)

目的[3]:掌握有理函数分式分解原则及三角有理式化简(万能代换与奇偶优先降幂策略)

目的[4]:熟练掌握定积分三大神技(对称性与周期性、区间再现公式、华里士点火公式)，实现高难度定积分快速降维

目的[5]:深刻掌握变上限积分求导莱布尼茨准则与含参分离法则，严格掌握变限积分奇偶性与周期性传导定理

目的[6]:掌握反常积分计算技巧与内部瑕点强制拆分法则，全面掌握 $\Gamma$ 函数定义、收敛性证明、递推性质与高斯积分联立应用

## 1.基本积分公式与不定积分的凑微分法

###### **公式[1]:10组基本积分公式全景汇总与记忆框架**
...
>
>(2) $\int k\,\mathrm{d}x = kx + C$ (特别地，$\int \mathrm{d}x = x + C$)
>
>(3) $\int x^a\,\mathrm{d}x = \frac{1}{a+1}x^{a+1} + C \quad (a \neq -1)$
>
>(4) $\int \frac{1}{x}\,\mathrm{d}x = \ln|x| + C$ (必须带绝对值符号，因定义域包含 $x < 0$)

[2]指数函数类：
>(1) $\int e^x\,\mathrm{d}x = e^x + C$
>
>(2) $\int a^x\,\mathrm{d}x = \frac{a^x}{\ln a} + C \quad (a > 0, a \neq 1)$

[3]基本三角函数一次项类：
>(1) $\int \cos x\,\mathrm{d}x = \sin x + C$
>
>(2) $\int \sin x\,\mathrm{d}x = -\cos x + C$ (注意负号)
>
>(3) $\int \tan x\,\mathrm{d}x = -\ln|\cos x| + C = \ln|\sec x| + C$
>
...
>
>(2) $\int \csc x\,\mathrm{d}x = -\ln|\csc x + \cot x| + C = \ln|\csc x - \cot x| + C$

[5]三角函数平方项类：
>(1) $\int \sec^2 x\,\mathrm{d}x = \int \frac{1}{\cos^2 x}\,\mathrm{d}x = \tan x + C$
>
>(2) $\int \csc^2 x\,\mathrm{d}x = \int \frac{1}{\sin^2 x}\,\mathrm{d}x = -\cot x + C$

[6]三角函数交叉乘积类：
>(1) $\int \sec x \tan x\,\mathrm{d}x = \sec x + C$
>
>(2) $\int \csc x \cot x\,\mathrm{d}x = -\csc x + C$

[7]反三角函数与平方和平方差(常数 $a > 0$)：
>(1) $\int \frac{1}{\sqrt{a^2-x^2}}\,\mathrm{d}x = \arcsin\frac{x}{a} + C$
>
>(2) $\int \frac{1}{a^2+x^2}\,\mathrm{d}x = \frac{1}{a}\arctan\frac{x}{a} + C$ (注意系数前方有 $\frac{1}{a}$)

[8]有理平方差分式：
>(1) $\int \frac{1}{a^2-x^2}\,\mathrm{d}x = \frac{1}{2a}\ln\left|\frac{a+x}{a-x}\right| + C$
>
>(2) $\int \frac{1}{x^2-a^2}\,\mathrm{d}x = \frac{1}{2a}\ln\left|\frac{x-a}{x+a}\right| + C$

[9]根号下平方和差反双曲类：
>(1) $\int \frac{1}{\sqrt{x^2+a^2}}\,\mathrm{d}x = \ln\left(x + \sqrt{x^2+a^2}\right) + C$
>
>(2) $\int \frac{1}{\sqrt{x^2-a^2}}\,\mathrm{d}x = \ln\left|x + \sqrt{x^2-a^2}\right| + C$
...
[2]基本积分公式：
>(1) $\int \sinh x\,\mathrm{d}x = \cosh x + C$
>
>(2) $\int \cosh x\,\mathrm{d}x = \sinh x + C$ (注意双曲函数互求积分无负号)
>
>(3) $\int \frac{1}{\cosh^2 x}\,\mathrm{d}x = \tanh x + C$

###### **方法[1]:不定积分的凑微分法(第一类换元法)**
...
[2]幂函数与根式类：
>(1) $x^n\,\mathrm{d}x = \frac{1}{n+1}\,\mathrm{d}(x^{n+1}) \quad (n \neq -1)$
>
>(2) $\frac{1}{\sqrt{x}}\,\mathrm{d}x = 2\,\mathrm{d}(\sqrt{x})$
>
>(3) $\frac{1}{x^2}\,\mathrm{d}x = -\,\mathrm{d}\left(\frac{1}{x}\right)$

[3]对数函数类：
>$$\frac{1}{x}\,\mathrm{d}x = \mathrm{d}(\ln x)$$

[4]指数函数类：
>(1) $e^x\,\mathrm{d}x = \mathrm{d}(e^x)$
>
>(2) $a^x\,\mathrm{d}x = \frac{1}{\ln a}\,\mathrm{d}(a^x)$

[5]正弦余弦一次因式类：
>(1) $\cos x\,\mathrm{d}x = \mathrm{d}(\sin x)$
>
>(2) $\sin x\,\mathrm{d}x = -\,\mathrm{d}(\cos x)$

[6]正割余割平方因式类：
>(1) $\sec^2 x\,\mathrm{d}x = \frac{1}{\cos^2 x}\,\mathrm{d}x = \mathrm{d}(\tan x)$
>
>(2) $\csc^2 x\,\mathrm{d}x = \frac{1}{\sin^2 x}\,\mathrm{d}x = -\,\mathrm{d}(\cot x)$

[7]正割余割积因式类：
>(1) $\sec x\tan x\,\mathrm{d}x = \mathrm{d}(\sec x)$
>
>(2) $\csc x\cot x\,\mathrm{d}x = -\,\mathrm{d}(\csc x)$
...
[10]正切余切分母对数类：
>(1) $\tan x\,\mathrm{d}x = -\frac{\mathrm{d}(\cos x)}{\cos x} = -\,\mathrm{d}(\ln|\cos x|)$
>
>(2) $\cot x\,\mathrm{d}x = \frac{\mathrm{d}(\sin x)}{\sin x} = \mathrm{d}(\ln|\sin x|)$
...
>
>[1]抓内层导数: 先看被积表达式中最复杂、最不规整的复合结构(如 $\sqrt{\dots}$, $e^{\dots}$, $\ln(\dots)$, $\arctan(\dots)$)，观察其余部分是否正好为内层函数的导数。
>
>[2]平衡系数: 导数中产生的常数倍数，必须在外侧同步乘以倒数予以抵消，确保等式严格恒等。
>
>[3]化整为零: 遇到分式结构，若分子有多项，应首先考虑拆项，将一个看似庞杂的积分拆成若干个标准的凑微分积木。
...
[2]根式代换(直接消根)：
>(1) 单个一次根式: 若式中含有 $\sqrt[n]{ax+b}$，直接令 $t = \sqrt[n]{ax+b}$，则 $x = \frac{t^n-b}{a}$，$\mathrm{d}x = \frac{n}{a}t^{n-1}\mathrm{d}t$。
>
>(2) 分式型一次根式: 若含有 $\sqrt[n]{\frac{ax+b}{cx+d}}$，令 $t = \sqrt[n]{\frac{ax+b}{cx+d}}$，反解出 $x$ 后求微分化为有理分式。
>
>(3) 多个不同方根: 若式中同时含有 $\sqrt{x}$ 与 $\sqrt[3]{x}$，取根指数的最小公倍数，令 $x = t^6$。

[3]倒代换(高次分母降次神技)：
>(1) 适用特征: 被积函数为分式，且分母的次数明显高于分子的次数(通常高出2次及以上)，尤其是分母外侧含有 $x$ 或 $x^n$，内侧含有二次根式 $\sqrt{ax^2+bx+c}$。
>
>(2) 代换法则: 令 $x = \frac{1}{t}$，则 $\mathrm{d}x = -\frac{1}{t^2}\,\mathrm{d}t$。
>
>(3) 妙用机制: 分母中的 $x^k$ 会化为 $\frac{1}{t^k}$，微分中的 $\frac{1}{t^2}$ 可以直接将分子分母同乘以 $t$ 的高次方，原本处于根号内部的高次项瞬间被提炼至分子，从而转化为极容易求解的标准凑微分或反常积分形式。
...
[1]适用对象：
>(1) "降幂型": 多项式与指数函数、正弦函数、余弦函数的乘积，多项式经过若干次求导后导数变为0。
>
>(2) "循环型": 指数函数与正余弦函数的乘积。
...
>
>(3) 单二次因式 $(x^2+px+q)$ (判别式 $\Delta = p^2 - 4q < 0$):
...
[3]系数确定的三大速算技巧：
>(1) 赋值法(极点覆盖法/留数思想): 对不重实根因式，两边同乘该分母后令其根代入，瞬间秒杀待定系数。
>
>(2) 特殊值带入法: 给 $x$ 赋予特殊数值(如 $x=0, 1, -1$)，联立简单一元一次方程求解。
>
>(3) 比较最高次项系数法: 观察两端 $x$ 最高次项与常数项，快速锁定未知数。

[4]4种最简有理分式的标准积分公式：
>(1) $\int \frac{A}{x-a}\,\mathrm{d}x = A\ln|x-a| + C$
>
>(2) $\int \frac{A}{(x-a)^k}\,\mathrm{d}x = -\frac{A}{k-1}\frac{1}{(x-a)^{k-1}} + C \quad (k > 1)$
>
>(3) $\int \frac{Bx+C}{x^2+px+q}\,\mathrm{d}x$:
...
>
>(4) $\int \frac{1}{(x^2+a^2)^m}\,\mathrm{d}x$:
...
[1]公式成立的严密性条件：
>(1) 被积函数 $f(x)$ 必须在整个闭区间 $[a, b]$ 上连续(至多包含有限个第一类间断点，此时需分段积分)。
>
>(2) 原函数 $F(x)$ 必须在整段闭区间 $[a, b]$ 上处处连续可导且满足 $F'(x) = f(x)$。
>
...
>
>(1) 换被积函数: $f(x) \to f[\varphi(t)]$；
>
>(2) 换微分: $\mathrm{d}x \to \varphi'(t)\,\mathrm{d}t$；
>
>(3) 换积分上下限: $x=a \implies t = \alpha$，$x=b \implies t = \beta$ (满足 $\varphi(\alpha)=a, \varphi(\beta)=b$)。
>
>定积分换元的核心优势在于**换限不回代**，只要新变量上下限正确，直接代入新限结算即可，彻底免去了不定积分中复杂的几何三角形回代步骤。
>
...
>
>(1) 若 $f(x)$ 为奇函数($f(-x) = -f(x)$)，则：
>
>$$\int_{-a}^a f(x)\,\mathrm{d}x = 0$$
>
>(2) 若 $f(x)$ 为偶函数($f(-x) = f(x)$)，则：
>
>$$\int_{-a}^a f(x)\,\mathrm{d}x = 2\int_0^a f(x)\,\mathrm{d}x$$

[2]周期函数的定积分性质(积分起点无关性)：
...
[2]区间对称拓展法则(解决区间为 $[0, \pi]$ 与 $[0, 2\pi]$)：
>(1) 积分区间为 $[0, \pi]$ 时：
...
[2]被积函数含自变量 $x$ 的求导处理原则：
>对于 $F(x) = \int_a^{u(x)} f(x, t)\,\mathrm{d}t$，因积分变量是 $t$，自变量 $x$ 同时出现在上下限与被积函数内部。
>
>(1) 线性分离法: 若 $f(x, t)$ 中 $x$ 与 $t$ 呈线性加减分离(如 $(x-t)g(t)$ 或 $x g(t)$)，必须先利用定积分性质把 $x$ 提至积分号外侧：
>
>$$\int_0^x (x-t)g(t)\,\mathrm{d}t = x\int_0^x g(t)\,\mathrm{d}t - \int_0^x t g(t)\,\mathrm{d}t$$
>
>然后再利用乘积求导法则展开求导。
>
>(2) 换元消除法: 若 $x$ 与 $t$ 复合在一起无法直接分离(如 $f(x-t)$ 或 $f(xt)$)，必须先通过换元代换(令 $u = x-t$ 或 $u = xt$)将外层变量 $x$ 转移到积分限上，彻底净化被积表达式内部，方可动用求导准则。

###### **定理[1]:变限积分的奇偶性与周期性传导定理**

[1]奇偶性传导定理(极高频考点)：
>设 $f(x)$ 在定义区间上连续：
>
>(1) 若 $f(x)$ 为**奇函数**：
>>变上限积分 $F(x) = \int_0^x f(t)\,\mathrm{d}t$ 必为**偶函数**；
>>
>>更为重要的结论: **奇函数的全体原函数均为偶函数**！
>>
...
>
>(2) 若 $f(x)$ 为**偶函数**：
>>变上限积分 $F(x) = \int_0^x f(t)\,\mathrm{d}t$ 必为**奇函数**；
>>
>>更为重要的结论: **偶函数的原函数中，有且仅有一个是奇函数**(即当下限为 0 且常数 $C = 0$ 时的那个原函数)！
>>
...
>
>(1) 对无穷积分段 $\int_1^{+\infty} x^{\alpha-1}e^{-x}\,\mathrm{d}x$：
...
>
>(2) 对瑕积分段 $\int_0^1 x^{\alpha-1}e^{-x}\,\mathrm{d}x$：
