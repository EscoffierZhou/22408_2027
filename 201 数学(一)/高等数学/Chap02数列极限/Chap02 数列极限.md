...
>
>   Intuition: 数列可视为自变量为正整数的函数,**数列一定有无穷多项**

###### **2.数列的性质**

[1]数列和子列:从$\{a_n\}: a_1, a_2 \dots a_n$中选取无穷多项组成数列$\{a_{n_k}\}: a_{n_1}, a_{n_2} \dots a_{n_k}$
...
[2]等差数列:$a_n = a_1 +(n-1)d$	$S_n = \frac{n}{2}[2a_1 +(n-1)d] = \frac{n}{2}(a_1 + a_n)$

[3]等比数列:$a_n = a_1 q^{n-1}$	  $S_n = \frac{a_1(1-q^n)}{1-q}(q \neq 1)$或$= na_1(q=1)$；等比级数:$1+r+r^2 \dots r^n = \frac{1-r^n}{1-r}(r \neq 1)$

[4]单调数列:若对所有正整数$n$,有$a_{n+1} \ge a_n(a_{n+1} \le a_n)$,则称数列为单调数列

[5]严格单调:若对所有正整数$n$,有$a_{n+1} > a_n(a_{n+1} < a_n)$,则称数列为严格单调

[6]有界数列:对所有正整数$n$,存在正实数$M$,有$|a_n| \le M$,则$\{a_n\}$为有界数列
...
[7]$\{(1+\frac{1}{n})^n\}$:单调递增有上界,且$\lim_{n \to \infty}(1+\frac{1}{n})^n = e$
...
>
>   区别:函数**局部有界**；数列**全局有界**(任意一项都有$|a_n| \le M$)
>
>   ​    函数**局部保号(气泡)**；数列**后面长保号(射线)**(须存在正整数$N, n>N$有$a_n>0$)
...
保号性\[1]:(全$\to$局部) 设$\lim_{n \to \infty} x_n = A > 常数b$(或$<常数b$),则存在$N > 0$,当$n > N$时有$x_n > b$(或$<b$)
...
###### **[2]海涅定理(归结原则):**
**<font color=deeppink>数列特点:题干中有显式通项公式,且内部不包含$n!$或$(-1)^n$等离散特有属性</font>**
...
###### **[4]压缩映射原理(纵向展开的,只给了递推关系,动点越来越接近靶心)**
...
**步骤[1]:求导:先对$f(x)$求导得到$f'(x)$(能量波动形式):** 

>   $f'(x)>0$（能量单向变化)	$\to$**单调有界准则**
>
>   $f'(x)<0$，$f(x)$震荡（能量左右乱跳)$\to$**压缩映射定理** 
>
...
**步骤[2D]\($f(x)$十分难算):包含超越式/分段函数/$x_{n+1}$难型(不可导)**
...
###### **[6]\(柯西视角)${x_n}$的收敛于$a$的速度**
**原文:** 设数列$\{x_n\}, \{y_n\}$在$n \to \infty$过程中同时趋于$A$
...
###### **[7]\(黎曼积分)定积分与数列极限$\Rightarrow$(夹逼拓展) 黎曼积分 (巨大的求和符号)**
**夹逼拓展:** 

情况[1]$n$的最高次幂和游走变量$i$**不同阶**做法: 用夹逼:不提取,用Max/Min找上下界（夹逼)

情况[2]$n$的最高次幂和游走变量$i$**同阶**做法:   用积分:提取$\frac{1}{n}$，剩下必然是$\frac{i}{n}$
...
**Eg:$\lim_{n \to \infty} (\frac{n}{n^2+1^2} + \frac{n}{n^2+2^2} + \dots + \frac{n}{n^2+n^2})$**

**解:** 游走变量$1^2, 2^2 \dots n^2$，所以$I = \frac{n}{n^2+i^2} \Rightarrow \sum_{i=1}^n \frac{n}{n^2+i^2}$($i$同阶,$n$同阶)
...
**Eg:$\lim_{n \to \infty} (\frac{1}{n^2+n+1} + \frac{2}{n^2+n+2} + \dots + \frac{n}{n^2+n+n})$**

**解:**$S_n = \sum_{i=1}^n \frac{i}{n^2+n+i} \Rightarrow$提$\frac{1}{n}$:$S_n = \frac{1}{n} \sum_{i=1}^n \frac{n \cdot i}{n^2+n+i} = \frac{1}{n} \sum_{i=1}^n \frac{i/n}{1+1/n+i/n^2}$($i$的幂次$\neq n$的最高次，积分失效$\to$夹逼)
...
###### **[8]Oth\(Stolz定理)处理纯离散未定式**

**<font color=deeppink>数列特点:一定是分式结构$\frac{x_n}{y_n}$,[分子通常是无法求导的离散累加和(如$\sum$)],[分母单调趋于$\infty$]</font>**

**定理内容($\frac{*}{\infty}$型):** 设$\{y_n\}$严格单调递增,且$\lim_{n \to \infty} y_n = +\infty$,若$\lim_{n \to \infty} \frac{x_{n+1}-x_n}{y_{n+1}-y_n} = A$(A可为实数、$\pm\infty$),则$\lim_{n \to \infty} \frac{x_n}{y_n} = A$

**SOP(我才不用斯托尔兹降维法):直接夹逼**
