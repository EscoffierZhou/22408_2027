...
\begin{center}
    ...
    {\color{TitlePurple}\fontsize{24}{28}\selectfont\bfseries 第十一章 一元积分学的应用(积分等式和积分不等式)}\\[0.6cm]
...
\begin{itemize}
  \item 目的[1]:深刻理解定积分中值定理与广义定积分中值定理的严格柯西中值定理证明，厘清连续型介值定理联用机理与中值依赖性 $\xi = \xi(x), \xi = \xi(n)$ 的易错陷阱
\end{itemize}
\end{targetbox}

\begin{targetbox}[本章复习目标与核心知识图谱]
\begin{itemize}
  \item 目的[2]:熟练掌握变限积分与导数中值定理（罗尔、拉格朗日、柯西）的深度嵌套转化体系，掌握多点导数符号判别与二阶中值定理综合推导
\end{itemize}
\end{targetbox}

\begin{targetbox}[本章复习目标与核心知识图谱]
\begin{itemize}
  \item 目的[3]:融会贯通积分等式证明的五大王牌技巧（辅助函数构建法、两端求导微分方程法、变上限积分变换法、泰勒展开内插积分法、分部积分降阶消项法）
\end{itemize}
\end{targetbox}

\begin{targetbox}[本章复习目标与核心知识图谱]
\begin{itemize}
  \item 目的[4]:系统掌握夹逼准则求积分极限的核心技术路线（函数有界放缩、局部多项式放缩、周期跳跃间断点阶梯放缩）与普遍性定理 $\lim\limits_{n\to\infty} \int_a^b x^n f(x)\,\mathrm{d}x = 0$
\end{itemize}
\end{targetbox}

\begin{targetbox}[本章复习目标与核心知识图谱]
\begin{itemize}
  \item 目的[5]:彻底掌握积分不等式证明的四大神技（构造辅助函数求导单调性法、被积函数局部放缩法与切线不等式、柯西-施瓦茨积分不等式、变量代换对称消除法）
\end{itemize}
\end{targetbox}

\begin{targetbox}[本章复习目标与核心知识图谱]
\begin{itemize}
  \item 目的[6]:建立从定积分基本性质、牛顿-莱布尼茨公式到绝对值积分估计与能量积分控制的完整考研解题方法论
\end{itemize}
\end{targetbox}


\section{1.积分中值定理体系及其深度推导}


\begin{equation*}
\int_a^b f(x)\,\mathrm{d}x = f(\xi)(b-a)
\end{equation*}
\begin{thmbox}[定理[1]: 第一定积分中值定理与连续介值性]
...
\end{equation*}
\begin{thmbox}[定理[2]: 广义积分中值定理（加权中值定理）与柯西中值定理推导]
...
\textbf{[2]} 被积函数含参数 $n$ 时中值的动态性：
在积分 $\int_a^b f_n(x)\,\mathrm{d}x = f_n(\xi_n)(b-a)$ 中，中值 $\xi_n$ 随着正整数 $n$ 的变动而变动，即 $\xi_n = \xi(n)$。

致命易错点：对于积分 $\int_1^2 \mathrm{e}^{-x^n}\,\mathrm{d}x = \mathrm{e}^{-\xi_n^n}(2-1) = \mathrm{e}^{-\xi_n^n}$，虽然 $1 < \xi_n < 2$，但由于我们完全不知道序列 $\xi_n$ 逼近 1 的收敛速度，\textbf{绝对不能}主观臆断 $\lim\limits_{n\to\infty}\mathrm{e}^{-\xi_n^n} = 0$！必须使用被积函数放缩法与夹逼准则严格求解。

\end{thmbox}
\begin{thmbox}[模型[1]: 变限积分中值与微分中值定理的嵌套转化链]

考研高分核心大题往往将“积分中值定理”作为第一层踏板，将“微分中值定理（罗尔、拉格朗日、柯西）”作为第二层或第三层推导工具，形成链式推理：

\textbf{[1]} “积出一值，再寻多导”转化模式：
给定包含积分的等式条件（如 $\int_a^b f(x)g(x)\,\mathrm{d}x = C$），先利用积分中值定理求出区间内某点 $\eta \in (a, b)$ 处的函数值 $f(\eta)$。

将点 $\eta$ 与原题中给定的端点值（如 $f(a), f(b)$）相组合：
\textbf{(1)}  \textbf{若三点函数值相等}（$f(a) = f(\eta) = f(b)$）：在 $[a, \eta]$ 和 $[\eta, b]$ 上分别使用罗尔定理，得到两导数零点 $f'(\xi_1) = f'(\xi_2) = 0$，再对 $f'(x)$ 在 $[\xi_1, \xi_2]$ 上使用罗尔定理，即得二阶导数零点 $f''(\xi) = 0$。
\textbf{(2)}  \textbf{若三点函数值不等}：分别在两段小区间上使用拉格朗日中值定理得到不同符号的导数值 $f'(\xi_1) > 0, f'(\xi_2) < 0$，再次对导函数应用拉格朗日中值定理，推导出二阶导数符号 $f''(\xi) < 0$。

\end{thmbox}

\section{2.积分极限与夹逼准则核心解题法}

\begin{thmbox}[方法[1]: 夹逼准则求定积分极限的标准三部曲]
...
\textbf{[1]} 标准三步法实施流程：
\textbf{(1)}  \textbf{局部不等式放缩}：利用被积函数在积分区间上的单调性、最值、初等不等式（如 $\mathrm{e}^u \geqslant 1+u$、$\ln(1+u) \leqslant u$、$|\sin u| \leqslant |u|$）建立被积函数的上下夹逼界：
...
\textbf{(2)}  \textbf{逐项积分求值}：利用定积分的保序性两端同时积分：
...
\textbf{(3)}  \textbf{两端取极限锁定}：若 $\lim\limits_{n\to\infty}\int_a^b \phi_n(x)\,\mathrm{d}x = \lim\limits_{n\to\infty}\int_a^b \psi_n(x)\,\mathrm{d}x = L$，则原积分极限必为 $L$。
...
\end{equation*}
\begin{thmbox}[定理[3]: 多项式乘积型积分极限的普遍收敛定理]
...
\end{thmbox}
\begin{thmbox}[方法[2]: 含周期/跳跃间断点函数的积分极限处理技术]
...
\textbf{(1)}  \textbf{分母单调放缩}：$\frac{1}{n+1} < \frac{1}{x} \leqslant \frac{1}{n}$；

\textbf{(2)}  \textbf{分子面积累加}：根据函数周期性 $T = 1$，
...
\textbf{(3)}  \textbf{不等式同向相乘}：两边不等式各项均为正，相乘得到上下界，当 $n \to \infty$ 时两端极限一致，从而定出积分平均极限值。

\end{thmbox}

\section{3.积分等式证明的五大王牌方法}

\begin{thmbox}[方法[3]: 辅助函数与零点定理/罗尔定理法]
...
\end{thmbox}
\begin{thmbox}[方法[4]: 两端求导微分方程法（变上限积分转化）]

\textbf{[1]} 消除积分符号，归结为常微分方程初值问题：
...
\end{thmbox}
\begin{thmbox}[方法[5]: 分部积分降阶消项法]

\textbf{[1]} 利用分部积分法将积分等式向高阶导数或低阶函数转移：
...
典型考研应用：
\textbf{(1)}  出现 $f''(x)$ 与 $f(x)$，连续使用两次分部积分法，并巧妙选取代数多项式因式使其端点值为 0，从而使边际项 $[uv]_a^b$ 消失；
\textbf{(2)}  出现三角震荡因子 $\sin nx$ 或 $\cos nx$，凑微分降出因子 $\frac{1}{n}$，使求极限或放缩变得清晰透明。

\end{thmbox}
\begin{thmbox}[方法[6]: 泰勒展开内插积分法]

\textbf{[1]} 多项式局部代换与余项积分：
...
\section{4.积分不等式证明的四大王牌方法}

\begin{thmbox}[方法[7]: 构造辅助函数求导单调性法]

\textbf{[1]} 上限变量化与单调性传递准则：
若要证明 $[a, b]$ 上的积分不等式 $A \leqslant B$：

\textbf{(1)}  将不等式一端的定积分上限改为动点变量 $x \in [a, b]$；

\textbf{(2)}  移项构造单变量辅助函数 $F(x)$，使得 $F(a) = 0$；

\textbf{(3)}  对 $F(x)$ 求导计算 $F'(x)$，利用题目中的增减性、不等式条件判定 $F'(x)$ 在 $(a, b)$ 内的符号；

\textbf{(4)}  若 $F'(x) \geqslant 0$（单调递增），则由 $x > a \implies F(x) \geqslant F(a) = 0$；特别地，取 $x = b$ 即证得 $F(b) \geqslant 0$。

\end{thmbox}
\begin{thmbox}[方法[8]: 被积函数局部放缩法与切线不等式]
...
\end{thmbox}
\begin{thmbox}[方法[9]: 柯西-施瓦茨（Cauchy-Schwarz）积分不等式秒杀法]
...
\end{thmbox}
\begin{thmbox}[方法[10]: 变量代换（区间再现与对称化）消除法]
