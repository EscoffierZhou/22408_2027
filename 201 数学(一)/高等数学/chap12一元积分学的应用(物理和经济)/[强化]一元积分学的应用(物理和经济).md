...
目的[1]:攻克非均匀密度介质与复杂异形旋转体容器抽水做功的高阶变力模型，掌握积分上下限参数化与变限微积分技巧

目的[2]:深入解析半无限长细棒、无限大平面薄板对空间质点引力矢量的反常积分模型与收敛性，领悟对称性相消与角度代换在空间引力场中的奇迹

目的[3]:攻克铅直平板静水压力中心（Center of Pressure）偏离形心的物理机制与截面二阶惯性矩（平行移轴定理）解析，掌握流体静力学压心严格位置公式

目的[4]:系统掌握空间刚体转动惯量微元与平行轴定理（Huygens-Steiner 定理）的微积分严格证明，打通一元积分与二重积分惯性矩的内在联系

目的[5]:攻克古鲁金第二定理（Pappus-Guldin 旋转体体积定理）与第一定理的联合运用，建立“形心 - 体积 - 侧面积 - 抽水功 - 静水压”五位一体秒杀网络

目的[6]:攻克带有需求价格弹性的变弹性利润最大化微分方程模型与垄断市场中税收无谓损失（Deadweight Loss）与社会总剩余最优化分析

## 1.非均匀密度介质与复杂异形容器的高阶抽水变力做功

###### **模型[1]:密度随深度线性变化的盐水池抽水做功微元模型**
...
## 2.反常万有引力场与连续介质反常引力积分

###### **模型[3]:半无限长均质细棒对空间任意质点的引力矢量反常积分与 $45^\circ$ 对称奇迹**
...
>
>结论：**无论质点到细棒端点的距离 $d$ 为多大，半无限长细棒对质点的合万有引力方向恒定与坐标轴夹角为 $45^\circ$！** 这一反常积分性质在全国硕士研究生入学考试中具有极高的理论美感与考察价值。

###### **模型[4]:无限大均质平面薄板对轴线上质点的反常引力与恒定场**
...
>
>结论：**无限大均质平板产生的引力场是一个严格的匀强引力场，其引力大小与质点到平板的垂直距离 $z$ 完全无关！** 这与电磁学中无限大均匀带电平板的匀强电场 $E = \frac{\sigma}{2\varepsilon_0}$ 具有完全同构的微积分数学本质。

## 3.液体压力中心（压力作用点）偏离形心的二阶矩模型

###### **模型[5]:压力中心坐标公式与截面惯性矩（平行移轴定理渗透）**

在基础部分我们证明了单侧总压力等于形心处压强乘以面积 $P = p(\bar{y}) \cdot S$。但液体对平板各处的推力并不均匀（越深压强越大），因此**合水压力的等效作用点（压力中心 Center of Pressure）并不位于形心，而是必然位于形心下方**。
...
[2]与截面惯性矩与平行移轴定理的深刻联系：
>在截面几何学中：
>
>(1) **一阶静矩**：$S_x = \int_a^b y w(y)\,\mathrm{d}y = \bar{y} \cdot S$（$\bar{y}$ 为形心深度，$S$ 为平板总面积）；
>
>(2) **二阶惯性矩**：$I_x = \int_a^b y^2 w(y)\,\mathrm{d}y$ 为平板对水面轴的转动惯性矩。
>
>根据力学与微积分的**平行移轴定理**：
>
...
[3]压力中心三大核心物理推论：
>(1) **压心必在形心下方**：由于任何平面图形的二次轴矩 $I_{\bar{x}} > 0$ 且面积 $S > 0$，故 $\frac{I_{\bar{x}}}{\bar{y} S} > 0$，因此 $y_p > \bar{y}$ 恒成立！
>
>(2) **渐近逼近性**：当平板浸没非常深（即 $\bar{y} \to +\infty$）时，$\frac{I_{\bar{x}}}{\bar{y} S} \to 0$，压力中心逐渐与形心重合。
>
>(3) **矩形水闸平板特例**：
>
...
>
>结论：**顶边与水面齐平的垂直矩形闸门，其水压力等效作用点恰好位于水深 $\frac{2}{3}h$ 处！**

## 4.空间刚体转动惯量与平行轴定理的微积分严格证明

###### **定理[1]:平行轴定理（Huygens-Steiner 定理）的一般微积分证明**
...
>注意到：
>
>(1) $\int r_C^2\,\mathrm{d}m = I_C$（绕质心轴的转动惯量）；
>
>(2) $\int \mathrm{d}m = M$（刚体总质量）；
>
>(3) 由于原点选在质心位置，根据质心坐标定义：
>
...
###### **定理[2]:古鲁金第二定理（Pappus-Guldin 旋转体体积定理）推导与联合运用**
...
## 6.经济学深水区：需求弹性微分方程与社会福利优化

###### **模型[6]:需求价格弹性 $\eta(P)$ 与边际收益方程的联合积分**
...
[2]阿莫罗索-罗宾逊（Amoroso-Robinson）公式的微分推导：
...
>经济学深刻结论：
>
>(1) 若 $\eta > 1$（富有弹性），则 $MR > 0$，降价可增加总收益；
>
>(2) 若 $\eta < 1$（缺乏弹性），则 $MR < 0$，降价会导致总收益减少；
>
>(3) 若 $\eta = 1$（单位弹性），则 $MR = 0$，总收益达到极大值。

[3]由恒弹性反求需求函数与福利剩余微分方程：
...
###### **模型[7]:税收无谓损失（Deadweight Loss）与剩余面积极值优化**
...
[2]社会总福利的无谓损失（哈伯格三角形 Deadweight Triangle）：
>征税后：
>
>(1) 消费者剩余减少量 $\Delta CS$ 对应梯形面积；
>
>(2) 生产者剩余减少量 $\Delta PS$ 对应梯形面积；
>
>(3) 政府获得税收总额为 $T = t \cdot Q_t$。
>
...
>
>重要结论：**税收无谓损失与单位税额的平方 $t^2$ 成正比！** 这一结论是公共财政学与福利经济学中最著名的微积分极值法则。
